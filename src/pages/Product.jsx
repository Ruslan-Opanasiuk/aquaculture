...
export default function Product() {
  ...
  const productKey = productId || "forel"; // Дефолтне значення
  ...
  return (
    <div className="bg-[#F5F1E7] min-h-screen flex flex-col font-['Montserrat'] animate-fadeIn">
      ...
      <main className="bg-[#F5F1E7] flex-1 pb-[120px] flex flex-col gap-[100px] tablet:gap-[160px]">
        ...
        <section className="w-full flex justify-center">
          ...
          >
            <div className="flex flex-col tablet:grid tablet:grid-cols-2 mt-[60px] tablet:mt-[100px] gap-y-[40px]">
              ...
              {/* 1. TEXT INFO (RIGHT COLUMN ON DESKTOP) */}
              <div className="order-1 tablet:order-2 flex flex-col items-center text-center">
                <span
                  className="uppercase tracking-[0.12em] text-black/50"
                  style={{ fontSize: "13px" }}
                >
                  {product.subtitle}
                </span>
                ...
                <p
                  className="mt-[20px] tablet:mt-[16px] leading-[1.6] text-black/80 max-w-[400px]"
                ...
                <div className="hidden tablet:flex flex-col w-full">
                  ...
                  <div
                    className="mt-[80px] flex flex-col gap-[20px] leading-[1.6] text-black/80 text-left"
                    style={{ fontSize: "var(--body-font-size)" }}
                  >
                    {/* ІНДИКАТОРИ (DESKTOP) */}
                    {product.indicators && product.indicators.length > 0 && (
                      <div className="flex flex-col w-full mb-4 pb-4 border-b border-black/10">
              ...
              {/* 2. IMAGES (ANIMATION) - LEFT COLUMN ON DESKTOP */}
              <div
                className="
                  order-2
                  tablet:order-1
              ...
              <div className="order-3 tablet:hidden flex flex-col">
                ...
                <div
                  className="mt-[80px] flex flex-col gap-[20px] leading-[1.6] text-black/80 text-left items-start"
                  style={{ fontSize: "var(--body-font-size)" }}
                >
                  {/* ІНДИКАТОРИ (MOBILE) */}
                  {product.indicators && product.indicators.length > 0 && (
                    <div className="flex flex-col w-full mb-2 pb-4 border-b border-black/10">
                      {product.indicators.map((indicator, index) => (
                        <IndicatorRow
                          key={index}
                          label={indicator.label}
